...
#  cisco_config_retrieval.py - script will send SNMP queries to sets of addresses given a 
#  query string, then retrieve the configuration if the device is a cisco and it gives up 
#  the goods.  No failure checking to see if the string is valid
#
# example usage
# python snmp_test.py -c accnet_mgmt -t 10.21.1.110 -f ipaddresses.py
...
def read_type7_from_file(fname):
	results = []
	type7_lines = [i.strip() for i in open.readlines() if i.find("password 7") > 0]
	for line in type7_lines:
		results.append(fname+":"+i.strip()+" "+decode_cisco(i.split()[-1]))
	return results
...
class snmp_client:
	...
	def readSystemName(self, wholeMsg):
		...
		if errorStatus:
			...
			oid, val = SNMPModule.apiPDU.getVarBinds(rspPDU)[0]
			self._sysname = self._addr.replace(".","_")+"_"+val.prettyPrint()
			IO_LOCK.acquire()
